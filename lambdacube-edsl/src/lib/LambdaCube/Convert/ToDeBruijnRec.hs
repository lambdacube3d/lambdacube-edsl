module LambdaCube.Convert.ToDeBruijnRec {-(convertGPOutput)-} (convert) where

import GHC.TypeLits

import Debug.Trace

import LambdaCube.Language.APIType (FlatTuple(..),Frequency(..))
import LambdaCube.Language.DSLType (GPU,Tuple(..),TupleIdx(..))
import qualified LambdaCube.Language.APIType as T
import qualified LambdaCube.Language.DSLType as T hiding (Shadow)
...
import LambdaCube.Core.DeBruijnRec
import LambdaCube.Core.APIType
